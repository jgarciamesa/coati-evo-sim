...
cpp11::cpp_function('doubles sim_indels(size_t nchar, const doubles_matrix<> probs) {
    #include <iomanip>
    #include <iostream>
    #include <map>
    #include <random>
    ...
    std::mt19937 gen(rd());
    std::discrete_distribution<> m(probs[0,0], probs[0,1], probs[0,2]);
    std::discrete_distribution<> i(probs[1,0], probs[1,1], probs[1,2]);
    std::discrete_distribution<> d(probs[2,0], probs[2,1], probs[2,2]);
    size_t n{0};
    size_t prev{1};
    std::vector<size_t> path;
    while(n < n_char) {
        current = prev == 1 ? m(gen) : prev == 2 ? i(gen) : d(gen);
...
#            }
#        }
        path = sim_indels(nchar(seqA), transitionP)
